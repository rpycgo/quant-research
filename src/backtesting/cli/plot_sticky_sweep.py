...
def plot_sticky_sweep(
    perf_df: pd.DataFrame,
    sweep_df: pd.DataFrame,
    baseline: int,
    out_path: str,
    ) -> None:
    """Render Figure: two-panel sticky threshold sweep."""
    plt.rcParams.update({
    ...
    ax_a.text(baseline + 0.15, -df["sharpe"].max() * 0.1,
              f"baseline\n($d_{{min}}={baseline}$)",
    ...
    ax_a.set_xlabel(
        r"Minimum Persistence Length $d_{\min}$", fontsize=10)
    ...
    ax_a.set_ylim(-0.5, df["sharpe"].max())
    ax_a.set_facecolor("#fafafa")
    ax_a.set_title('(A) Economic performance', y=1.05)

    # trade count annotation
    for di, si, ti in zip(d, df["sharpe"], df["trades"]):
        va = 'bottom' if si >= 0 else 'bottom'
        y = si + 0.035 if si >= 0 else si - 0.11
        ax_a.text(di, y, f"n={ti}",
                  ha="center", va=va, fontsize=7.2, color=C_GRAY)
    ...
    ax_b.set_title('(B) Delay vs. false-flip suppression', y=1.05)
...
def main() -> int:
    ...
    parser.add_argument(
        "--out", default="results/plot/sticky_sweep.png",
    ...
    plot_sticky_sweep(perf_df, sweep_df, args.baseline, args.out)
